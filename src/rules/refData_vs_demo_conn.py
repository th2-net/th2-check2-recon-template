...
class Rule(rule.Rule):
    ...
    def description_of_groups(self) -> dict:
        return {
            'SS_LOG': MessageGroupDescription(single=True),
            'SS_CONN': MessageGroupDescription(single=True)
        }
    ...
    def group(self, message: ReconMessage, attributes: tuple, *args, **kwargs):
        ...
        if message_type == 'SecurityStatus' and ReconMessageUtils.get_value(message, 'SecurityStatusReqID') == "":
        ...
        if session_alias in ['demo-conn1', 'demo-conn2']:
            message.group_name = 'SS_CONN'
        elif session_alias in ['security_status.txt']:
            message.group_name = 'SS_LOG'

    def hash(self, message: ReconMessage, attributes: tuple, *args, **kwargs):
        trd_match_id = ReconMessageUtils.get_value(message, 'SecurityStatusReqID')
